...
**本项目用于天天象棋人机对局辅助，不维护、不提供后续适配或技术支持。** 仅支持仓库样本对应的固定棋盘。天天象棋更新、棋盘布局或棋子样式变化导致无法使用时，请自行使用 AI 编程工具修改源码适配、验证并重新编译安装，不要等待本项目更新。用途为人机练习与研究。
...
首页保持一个主操作：未连接时显示“开始”，收到棋研录屏画面后显示“继续指导”，画面中断超过三秒后显示“重新连接”。无设置开关、不用滑动；使用说明和实时状态放在右上角“使用帮助”二级页面。着法放大、起终点图示突出，并明确区分实时、回看和旧局面。棋盘背景与系统画中画圆角自然衔接，棋子保留安全内边距；应用不再叠加左侧直角木框。

1. 在真机打开 App，点击“开始”。接收器、识别和悬浮来源自动准备。
2. 弹出的系统录屏面板已指定“棋研录屏”，点击“开始直播”。这一次系统确认不能由 App 跳过。
3. 收到“棋研录屏”的有效画面后自动请求开启悬浮指导，再切到指定的人机棋盘，保持完整棋盘可见。系统显示正在录屏并不代表棋研已收到画面，其他应用的录屏不能替代“棋研录屏”。关闭过悬浮窗且画面仍正常接收时，返回首页点“继续指导”恢复。
4. 看到走法后在天天象棋中操作。点选棋子时可回看上一条走法，落子确认后显示下一阶段状态。
5. 从中局进入时保持上一步白色起点与落点标记可见，程序自动确认轮次；标记不清晰时等待下一次可确认的走子。
6. 画面中断时会暂停当前落子指引；超过三秒后，首页恢复“重新连接”入口。点击后在系统面板重新开启“棋研录屏”；从未收到画面时入口仍显示“开始”。本机接收监听发生临时错误会自动重试。
7. 结束时关闭系统录屏。
...
当前工程版本为 **1.1（2026091508）**，生成脚本与 Xcode 工程的构建号保持一致。需要 macOS、Xcode 和 iPhone，最低部署版本为 iOS 17。已验证的开发环境为 Xcode 26.6、iOS 26.5 模拟器，以及 iPhone 17 Pro Max / iOS 26.6.2。
...
本项目不维护。需要适配时，可将本仓库和新版本天天象棋的实际棋盘截图、录屏样本及问题描述交给自己的 AI 编程工具，参照下面的文件修改。AI 生成的改动也需要自行测试，确认红黑朝向、轮次和连续走子正确后再安装；本项目不承诺兼容后续版本。
...
测试覆盖真实本机 TCP 传输、独立坐标实图识别、红黑朝向、已走炮局面、棋规与走子历史、计算取消/超时、上一步标记与自动轮次、最新帧调度、选子期间走法保留，以及图形箭头与旧帧隔离。另覆盖双方连续悔棋、跨历史段恢复、悬浮启动/停止竞态与原生按钮整块命中。界面流程在 `.maestro/`，验证记录见 [一键入口与悔棋回归](docs/simple-start-takeback-validation.md)、[内部 UI 优化与真机安装](docs/overlay-ui-validation.md)、[圆角衔接回归](docs/pip-corner-validation.md) 和 [录屏接收与识别恢复验证](docs/recognition-recovery-validation.md)。

1508 版完整 XCTest 181 项通过，最后一次浮窗取消条件修改定向复测 17 项通过；首页与帮助/录屏入口的 Maestro 流程 2 条通过。包含选子、浮窗遮挡后恢复和继续走下一步的合成图像序列验证；合成样本与模拟器不能替代真实录屏对局。

运行诊断保存在 App 自己的 `Documents/`：

- `coach-diagnostics.json`：当前版本、录屏连接、识别/浮窗状态、局面、帧数和耗时。
- `coach-diagnostics-history.json`：最多 120 条状态变化，用于事后追查中断；帧数和耗时单独变化不会追加历史。

诊断不保存录屏照片或头像，文件留在本地、不上传到仓库。自动测试通过不代表所有天天象棋版本和真机布局均可用。
